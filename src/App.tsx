...
function TemplatesTab({
  ...
  useEffect(() => {
    const loadCentralGithubConfig = async () => {
      ...
      const localAutoSync = localStorage.getItem('github_autosync');

      if (localPat) {
        setGithubToken(localPat);
        setTempPat(localPat);
      }
      if (localRepo) {
        setGithubRepo(localRepo);
        setTempRepo(localRepo);
      }
      if (localBranch) {
        setGithubBranch(localBranch);
        setTempBranch(localBranch);
      }
      if (localAutoSync !== null) {
        const isAuto = localAutoSync === 'true';
        setGithubAutoSync(isAuto);
        setTempAutoSync(isAuto);
      }
      ...
      try {
        ...
        console.warn('Could not fetch central github config from server (using local config):', err);
      }
    };
